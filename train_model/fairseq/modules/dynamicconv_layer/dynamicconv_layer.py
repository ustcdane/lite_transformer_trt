...
#https://support.huawei.com/enterprise/en/doc/EDOC1100191776/546085bf/exporting-a-custom-operator
class dynamicconvFunction(Function):
    ...
    def forward(ctx, x, weights, padding_l):
        ...
        #outputs = torch.ops.DynamicconvSpace.dynamicconv_forward(x, weights, padding_l)
        variables = [x, weights]
        ctx.save_for_backward(*variables)
        #save values to check plugin
        #print(f'x:{x.shape} weights:{weights.shape} padding_l:{padding_l} outputs[0]:{outputs[0].shape}')
        #save_data = {'x':x.cpu().detach().numpy(), \
        #    'weights':weights.cpu().detach().numpy(), \
        #    'padding_l':padding_l,
        #    'outputs':outputs[0].cpu().detach().numpy()
        #}
        #ws=weights.shape
        #file_out = f'dynamicconvFunction-{ws[0]}-{ws[1]}-{ws[2]}-{ws[3]}.npz'
        #np.savez(file_out , **save_data)
        return outputs[0]
    
    @staticmethod
    def symbolic(graph, x, weights, padding_l):
        args = [x, weights]
        kwargs = {"padding_l_i": padding_l}
        return graph.op("DynamicconvSpace::dynamicconv_forward", *args, **kwargs)
    '''
    ''' 
