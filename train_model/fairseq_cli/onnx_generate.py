...
def _main(cfg: DictConfig, output_file):
    ...
    for sample in progress:
        ...
        ### onnx  test
        if len(models) == 1 and num_sentences == 0:
            print(f'+++++++sample:{sample}')
            import onnx
            model = models[0]
            model.eval()
            input_tuple = tuple(v for k, v in sample["net_input"].items() if 'target' not in k)
            ...
            input_names = ["src_tokens", "src_lengths"],   # the model's input names
            )
            '''
            output_names = ['output_tokens', 'output_scores'],
            dynamic_axes={
                "src_tokens": axes,
